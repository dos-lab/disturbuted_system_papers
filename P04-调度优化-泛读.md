...
//HERTI针对有deadline约束的、运行在异构嵌入式设备上的DL推理服务，使用DL方法降低算子划分的搜索空间，同时精确估算不同设备间的通信和计算延迟，相较于已有工作能满足异构、QoS的需要。此种方法仍然聚焦于整个layer的资源使用情况，其控制粒度、资源使用效率在引入算子多种切分方法后还能进一步提升。
